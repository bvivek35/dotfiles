...
function! AS_M_DetectActiveWindow (filename)
    ...
    let active_window = system('osascript -e ''tell application "Terminal" to every window whose (name contains "'.shortname.'")''')
    ""let active_window = substitute(active_window, '^window id \d\+\zs\_.*', '', '')
    let active_window = substitute(active_window, '^window id ', '', '')
    return active_window =~ '\d\d\d' ? active_window : ""
endfunction
...
function! AS_M_SwitchToActiveWindow (active_window)
    call system('osascript -e ''tell application "Terminal" to set frontmost of every window whose id is '.a:active_window.' to true''')
endfunction
